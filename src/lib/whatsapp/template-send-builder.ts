...
function buildHeaderComponent(
  ...
  // every send. Prefer explicit overrides; fall back to stored sample.
  // Meta Cloud API JSON schema constraint: `id` MUST be a numeric Meta ID,
  // while URLs (http/https) MUST be sent under `link`.
  ...
  if (params.headerMediaUrl?.trim()) {
    ...
    if (val.startsWith('http://') || val.startsWith('https://')) {
      mediaPayload = { link: val };
    } else {
      mediaPayload = { id: val };
    }
  } else if (template.header_handle?.trim()) {
    const val = template.header_handle.trim();
    if (val.startsWith('http://') || val.startsWith('https://')) {
      mediaPayload = { link: val };
    } else {
      mediaPayload = { id: val };
