...
async function handleStatusUpdate(status: {
  ...
  if (status.status === 'failed') {
    const statusWithErr = status as unknown as {
      errors?: Array<{ code?: number; title?: string; message?: string }>
    }
    const errObj = statusWithErr.errors?.[0]
    if (errObj) {
      const codeStr = errObj.code ? `(#${errObj.code}) ` : ''
      const msgStr = errObj.message || errObj.title || 'Delivery failed on Meta network'
      update.error_message = `${codeStr}${msgStr}`
    } else {
      update.error_message = 'Delivery failed on Meta network'
    }
  }
